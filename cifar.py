...
# parser.add_argument('--mixing_set', type=str, default='./fractals_and_fvis', required=True, help='Mixing set directory.')
...
def FreqTune(orig, preprocess):
    tensorize, normalize = preprocess['tensorize'], preprocess['normalize']
    transform = FreqTune_transform.FCM()
...
class FreqTuneDataset(torch.utils.data.Dataset):
  def __init__(self, dataset, preprocess):
      self.dataset = dataset
      self.preprocess = preprocess
  ...
  def __getitem__(self, i):
    x, y = self.dataset[i]
    return FreqTune(x, self.preprocess), y              ###########################
...
def main():
  ...
  # train_data = PixMixDataset(train_data, {'normalize': normalize, 'tensorize': to_tensor})
